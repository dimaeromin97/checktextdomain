module.exports = source => {
	let num,
		...
		emitToken = function(token, code, preventBuffer, l) {
			if (!preventBuffer && bufferType) {
				...
				const ret_push = code ? [code, token, l || line] : token;
		...
		splitString = function() {
			...
			buffer &&
				emitToken(buffer, tokens.T_ENCAPSED_AND_WHITESPACE, true, lineBuffer);
			buffer = '';
			lineBuffer = line;
		},
		...
		getTypeCasting = function() {
			...
			return match && match[1] && match[1].toLowerCase() in typeCasting
				? match
				: null;
		},
		...
		getNumber = function() {
			...
			if (match[2]) {
				...
				if (
					parseInt(isHex ? parseInt(match[2], 16) : match[2], 10) < 2147483648
				) {
	...
	if (this.php_js && this.php_js.phpParser) {
		...
		emitToken = function(token, code, preventBuffer, l) {
			var action =
				that.php_js.phpParser[
					typeof token === 'number' ? that.token_name(token) : token
				];
	...
	for (; i < length; i++) {
		...
		if (isWhitespace(ASCII)) {
			...
			ch === '#' ||
			(ch === '/' &&
				((nextch = source.charAt(i + 1)) === '*' || nextch === '/'))
		) {
			//Comment signs
			...
			//Variable
			if (
				(bufferType === 'heredoc' || bufferType === 'doubleQuotes') &&
				!isEscaped()
			) {
				splitString();
			...
			if (sym in symbols3chars) {
				...
				if (
					bufferType === 'html' &&
					symbols3chars[sym] === tokens.T_OPEN_TAG_WITH_ECHO
				) {
					emitBuffer();
				}
			...
			if (!bufferType && (ch === "'" || ch === '"')) {
				...
				if (
					(ch === '[' && lastToken === tokens.T_VARIABLE) ||
					(ch === ']' &&
						(lastToken === tokens.T_NUM_STRING ||
			...
			if (
				keywordsToken.hasOwnProperty(word) ||
				keywordsToken.hasOwnProperty(wordLower)
			) {
			...
			if (
				(bufferType === 'nowdoc' || bufferType === 'heredoc') &&
				word === heredocWord &&
				source.charAt(i - 1) === '\n' &&
				...
				complexVarPrevBuffer &&
				lastToken === tokens.T_DOLLAR_OPEN_CURLY_BRACES
			) {
